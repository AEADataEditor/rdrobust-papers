...
capture   cd "c\Replication_files"  //path to replication files

local c_date = c(current_date)
local cdate = subinstr("`c_date'", " ", "_", .)
log using "bootstrap_se_for_dynamic_Tables_8_9_logfile_`cdate'.log", replace text

local SIMUL=1000  //number of boostrap simulations
...
local controls "`continious' `dummies' `mneigborhood'"
 ...
 //prepare input file -if not ready

  
   clear
   set obs 10000
   foreach Z  in   average_sigma1_20 average_sigma1_30 average_sigma1_40 average_sigma1_50  `controls' {   //first set of boostrap
		gen   `Z'=. 
		}
 
     
 foreach Y in  delta_dynamic delta_static delta_pi delta_dynamic0 delta0_pi delta_dynamic_  delta_d0nocom{	 				 
		foreach B in    price price1 price2/*price_RKDtr price_RKDun */ {
			gen `B'`Y'=.
  		}
	 }
 	 save ${basepath}/Replication_files/Results/boostraped_dynamic.dta, replace
 ...
 //prepare file to bootstrap
 
use ${basepath}/Replication_files/Data\RLMS.dta, clear
keep if age<66&age>=18
 save ${basepath}/Replication_files/Data\temp.dta, replace
...
use ${basepath}/Replication_files/Data\temp.dta, clear
keep if age<66&age>=18
...
**getting hermit polynomials for own characteritics
** to get predicition of peers
...
xi i.municipality_year
 ...
 compress
 save ${basepath}/Replication_files/dynamic_data.dta, replace
...
local i=`JJ'	
use ${basepath}/Replication_files/dynamic_data.dta, clear

//bsample, cluster(id psu censusd round)
 bsample, cluster(identificator)
...
local controls "`controls' average_sigma1_20 average_sigma1_30 average_sigma1_40 average_sigma1_50"
...
reg Y1 `controls'  _I* if binge==1 
	  ...
	  foreach Z of varlist   `controls' {   //first set of boostrap
		local `Z'_`S'=_b[`Z']
	 }
...
*outreg2 using `file' , ctitle("dynamic U(drink)=0" ) drop (_I*) nolabel bdec(3) se bracket append
...
gen delta_dynamic0= delta0_pi+ (delta0EV1- delta0V0)/`fi'
 
//no commitment on future price
gen delta_d0nocom= delta0_pi+ (delta0EV1- delta0V0) 
...
collapse id delta* fedok regulation_sum document_production document_retail premices_production excise_machine logprice_vodka_real excise_vodka run run_2 	city year round `mneigborhood' , by(municipality_year)
...
local iv "regulation_sum"
 ...
 capture drop _I*
 xi i.round i.fedok
 for var _Ife*: gen Xround=X*(round-15)

	 foreach Y of varlist delta_static delta_dynamic delta_pi delta_dynamic0 delta0_pi delta_dynamic_  delta_d0nocom{	 				 
		ivreg2 `Y' (logprice_vodka_real = `iv') _I* city `mneigborhood' if  year<=2008&year>=1995, robust
		*outreg2 using `file_report' , nolabel bdec(3) se bracket append addtext("Specs", "IV regional diffs") addstat("F-test", e(cdf))
				local  price`Y'_`S'=_b[logprice_vodka_real]
	  } 
	  ...
	  local iv "document_production document_retail premices_production excise_machine"
		...
		ivreg2 `Y' (logprice_vodka_real = `iv') _I* city `mneigborhood' if  year<=2008&year>=1995, robust
		*outreg2 using `file_report' , nolabel bdec(3) se bracket append addtext("Specs", "IV regional diffs") addstat("F-test", e(cdf))
				local  price1`Y'_`S'=_b[logprice_vodka_real]
	  } 
	
 capture drop _I*
 xi i.round i.id
 for var _Iid*: gen Xround=X*(round-15) 
 
 	 foreach Y of varlist delta_static delta_dynamic delta_pi delta_dynamic0 delta0_pi delta_dynamic_  delta_d0nocom{	 				 
		ivreg2 `Y' (logprice_vodka_real = `iv') _I* city `mneigborhood' if  year<=2008&year>=1995, robust
		*outreg2 using `file_report' , nolabel bdec(3) se bracket append addtext("Specs", "IV regional diffs") addstat("F-test", e(cdf))
				local  price2`Y'_`S'=_b[logprice_vodka_real]
	  } 
	  
 
 //writing coefficients in file
 use ${basepath}/Replication_files/Results/boostraped_dynamic.dta, clear
  
   foreach Z  in     `controls' {   //first set of boostrap
 		capture	replace `Z'=``Z'_`S'' in `S'
	}
   
   foreach Y in  delta_dynamic delta_static delta_pi delta_dynamic0 delta0_pi delta_dynamic_  delta_d0nocom{	 				 
		foreach B in    price  price1 price2 /*price_RKDtr price_RKDun */ {
 			capture	replace  `B'`Y'=``B'`Y'_`S'' in `S'	 
		}
	 }
	  di "**************************************"
      di  "simul= `S' "
 ...
 use ${basepath}/Replication_files/Results/boostraped_dynamic.dta , clear
 collapse (sd) average_sigma1_20- price2delta_d0nocom
 for var  average_sigma1_20- price2delta_d0nocom: lab var X "SE of X"
 save ${basepath}/Replication_files/Results/boostraped_dynamic.dta, replace
