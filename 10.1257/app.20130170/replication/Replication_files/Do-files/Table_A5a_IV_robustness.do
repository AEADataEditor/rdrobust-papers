...
log using "Table_A5a_IV_robustness_logfile_`cdate'.log", replace text
...
local file "${basepath}/Replication_files/Results/TableA5a_IV.xls"
...
collapse id delta* fedokrug regulation_sum excise_machine premices_production document_retail document_production  logprice_vodka_real excise_vodka run run_2 	city year round `mneigborhood' , by(municipality_year)
...
// RKD
 ...
 local mdelta_static  "Static"
 local mdelta_dynamic "Dynamic"
 local mdelta_dynamic_ "Dynamic"
 local mdelta_dynamic0 "Dynamic"
 local mdelta_dynamic0_ "Dynamic"
 
 local ndelta_static "no drink"
 local ndelta_dynamic "no drink" 
 local ndelta_dynamic_  "no drink"
 local ndelta_dynamic0 "drink" 
 local ndelta_dynamic0_  "drink"
 
 local cdelta_static "NA"
 local cdelta_dynamic "YES"
 local cdelta_dynamic_ "NO"
 local cdelta_dynamic0  "YES"
 local cdelta_dynamic0_  "NO"
 ...
 capture drop _I*
  ...
  //NOTE: PART OF TABLE A5a is reported in tables 7 and 8
  
  local append "replace"

 foreach Y of varlist  delta_dynamic   delta_dynamic0  {	 				 
 
  local iv "regulation_sum"

		xi i.round i.id
			ivreg2 `Y' (logprice_vodka_real = `iv') _I* city `mneigborhood' if  year<=2008&year>=1995, robust
			outreg2 using `file' , nolabel bdec(3)  keep(logprice_vodka_real) noaster stats(coef) append addtext("Model", "`m`Y''", "Normalization", "`n`Y''", "Permanent price change", "`c`Y''","Specification", "IV 1", 	"Regional FE", "YES" ) 
   local append "append"
 			ivreg2 `Y'_ (logprice_vodka_real = `iv') _I* city `mneigborhood' if  year<=2008&year>=1995, robust
			outreg2 using `file' , nolabel bdec(3)  keep(logprice_vodka_real) noaster stats(coef) append addtext("Model", "`m`Y''", "Normalization", "`n`Y''", "Permanent price change", "`c`Y'_'","Specification", "IV 1", 	"Regional FE", "YES" ) 

		for var _Iid*: gen Xround=X*round
			ivreg2 `Y' (logprice_vodka_real = `iv') _I* city `mneigborhood' if  year<=2008&year>=1995, robust
			outreg2 using `file' , nolabel bdec(3)  keep(logprice_vodka_real) noaster stats(coef) append addtext("Model", "`m`Y''", "Normalization", "`n`Y''", "Permanent price change", "`c`Y''","Specification", "IV 1", "Regional FE", "YES", "RegionalSpecific Trends", "YES") 
  	
			ivreg2 `Y'_ (logprice_vodka_real = `iv') _I* city `mneigborhood' if  year<=2008&year>=1995, robust
			outreg2 using `file' , nolabel bdec(3)  keep(logprice_vodka_real) noaster stats(coef) append addtext("Model", "`m`Y'_'", "Normalization", "`n`Y''", "Permanent price change", "`c`Y'_'","Specification", "IV 1", "Regional FE", "YES", "RegionalSpecific Trends", "YES") 
 
 local iv "document_production document_retail premices_production excise_machine"
	drop _I* 
	xi i.round i.fedokrug 
	for var _Ife*: gen Xround=X*round 
			ivreg2 `Y' (logprice_vodka_real = `iv') _I* city `mneigborhood' if  year<=2008&year>=1995, robust
			outreg2 using `file' , nolabel bdec(3)  keep(logprice_vodka_real) noaster stats(coef) append addtext("Model", "`m`Y''", "Normalization", "`n`Y''", "Permanent price change", "`c`Y''", "Specification", "IV 2" ,"Fedokrug FE", "YES", "FedokrugSpecific Trends", "YES") 
   
 			ivreg2 `Y'_ (logprice_vodka_real = `iv') _I* city `mneigborhood' if  year<=2008&year>=1995, robust
			outreg2 using `file' , nolabel bdec(3)  keep(logprice_vodka_real) noaster stats(coef) append addtext("Model", "`m`Y'_'", "Normalization", "`n`Y''", "Permanent price change", "`c`Y'_'", "Specification", "IV 2" ,"Fedokrug FE", "YES", "FedokrugSpecific Trends", "YES") 
   
 }
 

  foreach Y of varlist  delta_static {	 				 
 ...
 local iv "document_production document_retail premices_production excise_machine"
  ...
  xi   i.fedokrug i.round
	for var _Ife*: gen Xround=X*round 

gen no_excise=(excise_vodka ==.)
replace excise_vodka =0 if excise_vodka ==.
gen no_regul=year>=2009
replace regulation_sum=0 if year>=2009
 	 				 
	ivreg2 `Y' (logprice_vodka_real =  regulation_sum excise_vodka ) run run_2  no_regul no_excise _If* city `mneigborhood' if  year>=1995 , robust
	outreg2 using `file' , nolabel bdec(3)  keep(logprice_vodka_real) noaster stats(coef) append addtext("Model", "`m`Y''", "Normalization", "`n`Y''", "Permanent price change", "`c`Y''","Specification", "IV 3", "Fedokrug FE", "YES", "FedokrugSpecific Trends", "YES")   
