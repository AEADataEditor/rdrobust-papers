...
//  The purpose of the file is to create a STATA command, -dc_density-, which
...
// An example program showing how to use -dc_density- is given in the file
// dc_density_example.do

// JRM, 9/2008

// Update: Fixed bug that occurs when issuing something like
// dc_density Z if female==1, breakpoint(0) generate(Xj Yj r0 fhat se_fhat) graphname(dc_density_example.eps)
...
program dc_density, rclass
{
  ...
  syntax varname(numeric) [if/] [in/], breakpoint(real) GENerate(string) ///
    [ b(real 0) h(real 0) at(string) graphname(string) name(string) noGRaph title(string) graphregion(passthru)]

  // drop variables from last run
  foreach i in Xj Yj r0 fhat se_fhat {
    cap drop `i'
  }
  ...
  //Call MATA function
  mata: dc_densitysub("`R'", "`touse'", `breakpoint', `b', `h', `verbose', "`cellmpname'", "`cellvalname'", ///
        ...
        (line `cellsmname' `evalname' if `evalname' > `breakpoint', lcolor(black) lwidth(medthick))   ///
          (line `hi' `evalname' if `evalname' < `breakpoint', lcolor(black) lwidth(vthin))              ///
            (line `lo' `evalname' if `evalname' < `breakpoint', lcolor(black) lwidth(vthin))              ///
              (line `hi' `evalname' if `evalname' > `breakpoint', lcolor(black) lwidth(vthin))              ///
                (line `lo' `evalname' if `evalname' > `breakpoint', lcolor(black) lwidth(vthin)),             ///
                  xline(`breakpoint', lcolor(black)) legend(off) `name' `title' `graphregion'
    if ("`graphname'"!="") {
...
void dc_densitysub(string scalar runvar, string scalar tousevar, real scalar c, real scalar b, ///
