...
program define rdrobust, eclass
	...
	**************************** ERRORS
	...
	if ("`h'"=="0" & "`bwselect'"=="IK") {
		...
		qui rdbwselect `y' `x', c(`c') deriv(`deriv') p(`p') q(`q') bwselect(`bwselect') rho(`rho') kernel(`kernel') vce(`vce') precalc scaleregul(`scaleregul')
		local h = e(h_IK)
		local b = e(b_IK)
		local bwselect = "IK"
	}
		...
		disp in ye "Computing bandwidth selectors."
		qui rdbwselect `y' `x', c(`c') deriv(`deriv') p(`p') q(`q')  bwselect(`bwselect') rho(`rho') kernel(`kernel') delta(`delta') cvgrid_min(`cvgrid_min') cvgrid_max(`cvgrid_max') cvgrid_length(`cvgrid_length') `cvplot' vce(`vce') precalc scaleregul(`scaleregul')
		local h = e(h_CV)
		local b = e(h_CV)
		local bwselect = "CV"
	}
		...
		disp in ye "Computing bandwidth selectors."
		local bwselect = "CCT"
		qui rdbwselect `y' `x', c(`c') deriv(`deriv') p(`p') q(`q') matches(`matches') bwselect(`bwselect') rho(`rho') kernel(`kernel') vce(`vce') precalc scaleregul(`scaleregul')
		local h = e(h_CCT)
		local b = e(b_CCT)
	}
	...
	st_numscalar("mu_y_r_cl", tau_rp[`deriv'+1,1]) 
	st_numscalar("mu_y_l_cl", tau_lp[`deriv'+1,1])
	st_numscalar("mu_y_r_bc", tau_rp[`deriv'+1,1] - (tau_rq[`p'+2,1]*BiasConst_rp[`deriv'+1,1])*(`h'^(`p'+1-`deriv')/factorial(`p'+1)))
	st_numscalar("mu_y_l_bc", tau_lp[`deriv'+1,1] - (tau_lq[`p'+2,1]*BiasConst_lp[`deriv'+1,1])*(`h'^(`p'+1-`deriv')/factorial(`p'+1)))
...
*	st_numscalar("t_rb",  tau_bc[1,1]/se_rb)
	...
	if ("`fuzzy'"~="") {
		*********** First Stage ***************
		...
		sigmah_T_l = rdvce(Xh_l, Th_l, Th_l, `p', `h',`matches',"`vce'","`kernel'")
		sigmah_T_r = rdvce(Xh_r, Th_r, Th_r, `p', `h',`matches',"`vce'","`kernel'")
		sigmab_T_l = rdvce(Xb_l, Tb_l, Tb_l, `p', `h',`matches',"`vce'","`kernel'")
		sigmab_T_r = rdvce(Xb_r, Tb_r, Tb_r, `p', `h',`matches',"`vce'","`kernel'")
		...
		if (`b'>=`h'){
			Psi_T_lpq = cross(Xb_lp,whb_l:*sigmab_T_l:*wbb_l,Xb_lq);    
			Psi_T_rpq = cross(Xb_rp,whb_r:*sigmab_T_r:*wbb_r,Xb_rq)
		}
		else {
			Psi_T_lpq = cross(Xh_lp,whh_l:*sigmah_T_l:*wbh_l,Xh_lq);    
			Psi_T_rpq = cross(Xh_rp,whh_r:*sigmah_T_r:*wbh_r,Xh_rq)
		} 
		...
		st_numscalar("mu_T_r_cl", tau_T_rp[`deriv'+1,1]) 
		st_numscalar("mu_T_l_cl", tau_T_lp[`deriv'+1,1])
		st_numscalar("mu_T_r_bc", tau_T_rp[`deriv'+1,1] - (tau_T_rq[`p'+2,1]*BiasConst_rp[`deriv'+1,1])*(`h'^(`p'+1-`deriv')/factorial(`p'+1)))
		st_numscalar("mu_T_l_bc", tau_T_lp[`deriv'+1,1] - (tau_T_lq[`p'+2,1]*BiasConst_lp[`deriv'+1,1])*(`h'^(`p'+1-`deriv')/factorial(`p'+1)))
		...
		****************** Second Stage
		...
		sigmah_TY_l = rdvce(Xh_l, Yh_l, Th_l, `p', `h',`matches',"`vce'","`kernel'")
		sigmah_TY_r = rdvce(Xh_r, Yh_r, Th_r, `p', `h',`matches',"`vce'","`kernel'")
		sigmab_TY_l = rdvce(Xb_l, Yb_l, Tb_l, `p', `h',`matches',"`vce'","`kernel'")
		sigmab_TY_r = rdvce(Xb_r, Yb_r, Tb_r, `p', `h',`matches',"`vce'","`kernel'")
		...
		if (`b'>=`h'){
			Psi_TY_lpq = cross(Xb_lp,whb_l:*sigmab_TY_l:*wbb_l,Xb_lq);    
			Psi_TY_rpq = cross(Xb_rp,whb_r:*sigmab_TY_r:*wbb_r,Xb_rq)
		}
		else {
			Psi_TY_lpq = cross(Xh_lp,whh_l:*sigmah_TY_l:*wbh_l,Xh_lq);    
			Psi_TY_rpq = cross(Xh_rp,whh_r:*sigmah_TY_r:*wbh_r,Xh_rq)
		} 
	...
	************************************************
	...
	ereturn scalar mu_y_r_cl = mu_y_r_cl
	ereturn scalar mu_y_l_cl = mu_y_l_cl
	ereturn scalar mu_y_r_bc = mu_y_r_bc
	ereturn scalar mu_y_l_bc = mu_y_l_bc
	...
	*ereturn local selvce = "`vce'"
	...
	if "`fuzzy'"~="" {
		...
		ereturn scalar mu_T_r_cl = mu_T_r_cl
		ereturn scalar mu_T_l_cl = mu_T_l_cl
		ereturn scalar mu_T_r_bc = mu_T_r_bc
		ereturn scalar mu_T_l_bc = mu_T_l_bc
